...
leveraging stateful processing available in programmable switches while providing the operator with a view of one-big-switch (OBS) of persistent arrays. This abstraction is good at expressing network-wide policies that do not require explicit device control, e.g., DNS tunnel detection in a LAN. However, manually programming each device is necessary to accurately control all aspects of network performance. For instance, an operator may want to run active queue management (AQM), ECN, and DCTCP on particular switches. This is difficult to express in SNAP as it may automatically place the code on unintended devices. There is also no way to express queue-based measurement e.g., tracking an EWMA of queueing latencies on a particular switch. Maple and SNAP cannot express programmable-switch functionality that
affects network performance at fine time scales, e.g., packet scheduling,
congestion control, and load
