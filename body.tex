...
this creates performance overhead as packets that require stateful processing
...
leveraging stateful processing available in programmable switches. Stateful code placement accross the network is hidden from the operator who only sees one-big-switch (OBS) of persistent arrays. 
The OBS abstraction is good at expressing network-wide policies that do not require fine-grained device control, for example, DNS tunnel detection in a LAN. However, manually keeping track of and programming each device is necessary to accurately control all aspects of network monitoring and performance. For instance, an operator may want to run active queue management (AQM), ECN, and DCTCP on particular switches. This type of network-wide program is difficult to enforce in SNAP as it may automatically place the code on unintended devices. In addition, there is no way to express queue-based measurement in SNAP, for instance, tracking an EWMA of queueing latencies on a particular switch. Both Maple and SNAP cannot express programmable-switch functionality that
